...
def load_from_archive(fname):
    archived_data = torch.load(fname)
    edge_index = archived_data['edge_index']
    features = archived_data['features']
    labels = archived_data['labels']
    idx_train = archived_data['idx_train']
    idx_val = archived_data['idx_val']
    idx_test = archived_data['idx_val']
    sens = archived_data['sens']
    return edge_index, features, labels, idx_train, idx_val, idx_test, sens

def save_to_archive(fname, edge_index, features, labels, idx_train, idx_val, idx_test, sens):
    torch.save({'edge_index': edge_index, 'features': features,
