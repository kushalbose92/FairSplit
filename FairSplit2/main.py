"""
program: fairness with homophily based edge-splitting - two GNNs (FairSplit2)
...
from models.model import FairSplit2_model
...
result_cols = ['dataset', 'model_homo', 'model_hetero', 'init_lr', 'weight_decay', 'hiddden', 'dropout', 
               'epochs', 'avg_degree', 'isolated_nodes', 'h_ratio', 'acc', 'F1', 
               'D_sp', 'D_eo', 'acc_std', 'F1_std', 'D_sp_std', 'D_eo_std']
try:
...
model_homo = param_dict['model_homo']
model_hetero = param_dict['model_hetero']
...
for run in range(3):
    ...
    for sg in subgraphs:
        ...
        model = FairSplit2_model(param_dict, sg.num_edges, sg.num_non_edges)
        ...
        if run==0 and i==1: # plot training history only for first subgraph
            fname = f"{root}results/hist/{dataset_name}_{param_dict['init_lr']}"
            fname = fname+f"_{param_dict['weight_decay']}_{param_dict['dropout']}.png"
...
results_df.loc[len(results_df)] = [dataset_name, model_homo, model_hetero, param_dict['init_lr'], 
